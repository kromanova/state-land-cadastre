...
function createArea(req, res, next) {
    ...
    collection.save(area).then(
      ...
      meta => {
        ...
        var edge = {};
        var start = String('area/' + meta._key);
        var end = String('owner/' + area.owner_id);
        has_owner.save(edge, start, end);

        var has_registrar = req.db.edgeCollection('has_registrar');
        end = String('registrar/' + area.registrar_id);
        has_registrar.save(edge, start, end);
...
function deleteArea(req, res, next) {
    ...
    req.collections.area.remove(req.owner).then(() => {
      ...
      var start = String('area/' + req.params.id);
      var has_owner = req.db.edgeCollection('has_owner');
      var has_registrar = req.db.edgeCollection('has_registrar');
