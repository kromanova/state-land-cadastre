...
function deleteOwner(req, res, next) {
    ...
    req.models.owner.get(req.owner, function(err, result){
        ...
        if (err) {
            ...
            deleteOwnerDocuments(result, req);
            deleteOwnerAreas(result, req);
            result.remove(function (err) {
                if (err) {
                    req.log.warn(err, 'deleteOwner: unable to delete %s', req.owner);
                    next(err);
                } else {
                    res.send(204);
                    next();
    ...
    req.models.owner.all().remove(function (err) {
        if (err) {
            ...
            req.models.document.all().remove(function (err) {
                if (err) {
                    req.log.warn(err, 'deleteAllOwners: unable to delete documents');
                }
            });

            req.models.area.all().remove(function (err) {
                if (err) {
                    req.log.warn(err, 'deleteAllOwners: unable to delete areas');
                }
            });
