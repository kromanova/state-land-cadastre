...
function deleteRegistrar(req, res, next) {
    ...
    req.collections.registrar.remove(req.registrar).then(() => {
      ...
      var end = String('registrar/' + req.params.id);
      has_registrar.removeByExample({_to: end});

      var collection = req.db.collection('area');

      // removes all edges that refer to removing areas
      collection.byExample({registrar_id: parseInt(req.params.id)}).then(cursor => {
          cursor.all().then(areas => {
              ...
              for(var i = 0; i < areas.length; i++) {
                  ...
                  has_owner.removeByExample({_from: start});
              }

              collection.removeByExample({registrar_id: parseInt(req.params.id)});
          });
      });
